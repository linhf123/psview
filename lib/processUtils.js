  ...
  try {
    ...
    if (all) {
      ...
      if (process.platform === "win32") {
        // WMIC CSV 顺序: Node,CommandLine,ParentProcessId,ProcessId
        command = `wmic process get ProcessId,ParentProcessId,CommandLine /format:csv`;
      } else {
...
async function parseProcessOutput(output, platform) {
  ...
  for (const line of lines) {
    ...
    if (platform === "win32") {
      // Windows CSV格式解析: Node,CommandLine,ParentProcessId,ProcessId
